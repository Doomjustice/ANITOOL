...
class GcCog(commands.Cog):
    ...
    @commands.command()
    async def gc(self, ctx, *, name: str) -> None:
        ...
        while not self.spam_flag:
            try:
                await channel.edit(name=f"♰ {count} ♰ | ♰ {name} ♰ | {count}")
                self.bot.logger.info(f"Changed channel name to: ♰ {count} ♰ | ♰ {name} ♰ | {count}")
                count += 1
    ...
    @commands.command()
    async def gc_stop(self, ctx) -> None:
        ...
        try:
            ...
            temp_message = await ctx.send("`Use >bypass_limit to Unflag The spam!`")
    ...
    @commands.command()
    async def bypass_limit(self, ctx) -> None:
        """Stops limit"""
        try:
            await ctx.message.delete()
            self.spam_flag = False
            self.bot.logger.debug("Bypassed The Limit")
            temp_message = await ctx.send("`Bypassed Rate Limit`")
...
```""")
    ...
    async def start(self, ctx):
        """Start changing the group chat name."""
        if self.running:
            await ctx.send("The name changer is already running.")
            return
        self.spam_flag = False
        await ctx.send("`Starting the name changer...`")
        
        while not self.spam_flag: 
            for sentence in self.sentences:
                await ctx.guild.edit(name=sentence)
                await asyncio.sleep(5)  # Change name every 5 seconds
