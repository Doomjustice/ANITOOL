...
def get_sentences() -> List[str]:
    ...
    try:
        with open("assets/thick_of_it.txt", "r") as file:
            sentences = file.readlines()
            sentences = [sentence.strip() for sentence in sentences if sentence.strip()]
            return sentences
    except FileNotFoundError:
        print(f"[{Fore.RED}-{Fore.RESET}] Error: File 'thick_of_it.txt' not found.")
        return []
    except Exception as e:
        print(f"[{Fore.RED}-{Fore.RESET}] Error: {e}")
        return []

class SingCog(commands.Cog):
    ...
    @commands.command()
    async def sing(self, ctx, *, users: Optional[discord.User] = None) -> None:
        """Start singing."""
    ...
    @commands.command()
    async def stop_sing(self, ctx) -> None:
        """Stop the packing process."""
        try:
            await ctx.message.delete()
            self.stop_pack_flag = True

            self.bot.logger.debug("Stopped singing")
            temp_message = await ctx.send("`Stopped singing`")
    ...
    @commands.command()
    async def delay(self, ctx, new_delay: float) -> None:
    ...
    @commands.command()
    async def delete_ksi(self, ctx) -> None:
        """Delete ksi"""
        await ctx.message.delete()
