...
class CustomFormatter(logging.Formatter):
    ...
    def format(self, record):
        level_colors = {
            logging.DEBUG: (Style.BRIGHT + Fore.YELLOW, "?"),
...
class ANITOOL(commands.Bot):
    def __init__(self, prefix: str = ">", token_file_path: str = "./assets/token.env"):
        ...
        async def on_ready():
            ...
            self.logger.info(f"Logged in as {Style.BRIGHT + Fore.YELLOW}{self.user}{Fore.RESET}")
            self.logger.debug(f"Prefix: {Style.BRIGHT + Fore.YELLOW}{self.command_prefix}{Fore.RESET}")
    ...
    def _load_logging() -> None:
        ...
        logger = logging.getLogger("ANITOOL BY DJ")
    ...
    @staticmethod
    def _set_console_title(title: str = "ANITOOL") -> None:
    ...
    def _load_token(self) -> str:
        ...
        if os.path.exists(self.token_file_path):
            ...
            use_saved_token = (
                input(f"[{Style.BRIGHT + Fore.YELLOW}?{Fore.RESET}] Use saved token? (y/n): ")
                .strip()
                .lower()
            )
        ...
        return input(f"[{Style.BRIGHT + Fore.YELLOW}?{Fore.RESET}] Enter your token: ").strip()
    ...
    def _save_token(self, token: str) -> None:
        ...
        with open(self.token_file_path, "w") as file:
            file.write(token)
        print(f"[{Style.BRIGHT + Fore.YELLOW}+{Fore.RESET}] Token saved.")
        ...
        for filename in os.listdir("./cogs"):
            if (
                ...
                try:
                    ...
                    print(f"[{Style.BRIGHT + Fore.YELLOW}+{Fore.RESET}] {Fore.RESET}Loaded {Style.BRIGHT + Fore.YELLOW + cog + Fore.RESET}.")
    ...
    def main(self) -> None:
        ...
        print(f"[{Style.BRIGHT + Fore.YELLOW}+{Fore.RESET}] Token is {Style.BRIGHT + Fore.YELLOW}Valid.{Fore.RESET}")
        ...
        if self.token != self.saved_token:
            remember_token = (
                input(f"[{Style.BRIGHT + Fore.YELLOW}?{Fore.RESET}] Remember token? (y/n): ")
                .strip()
                .lower()
            )
            if remember_token == "y":
